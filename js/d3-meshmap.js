	...
	TPChart.prototype.render = function (nodes, lines) {
		...
		_g_lines.append("line")
			...
			.style('stroke-dasharray', function (d) {
				if ( "undefined" != typeof(d.ifname) &&  d.ifname != "Eth") {
					return '5,5';
				}
			});
		...
		_g_nodes.append("text")
			.text(function (d) {
				if( 'undefined' == typeof(d.hostname) || "" == d.hostname || "\"\"" == d.hostname || "''" == d.hostname )
					return 'unknown';
				else
					return d.hostname;
			})
   ...
   function TPChart(option) {
	...
	function createInfoTip(node) {
		...
		if (node.type == "agent"){
			...
			//$(".dialog-footer").html("Status: "+node.info.netstatus+ "<br>RSSI: "+node.info.rssi+"</br>Security Type: "+security_2g+"(2.4G)&nbsp;&nbsp;&nbsp;"+security_5g+"(5G)<br>Channel: "+node.info.ch_2g+"(2.4G)&nbsp;&nbsp;&nbsp;"+node.info.ch_5g+"(5G)</br>WiFi Mode: "+node.info.mode_2g+"(2.4G)&nbsp;&nbsp;&nbsp;"+node.info.mode_5g+"(5G)<br>Link Speed: "+node.info.link_speed+"</br>");
		}else{
			$(".dialog-footer").html("<br></br><br></br><br>Status: "+node.info.netstatus+"</br>");
		}
		var html = '<div class="node-info"><ul>';
		html += '<li><span class="info-title">Status:</span><span class="info-content">'+node.info.netstatus+'</span></li>';
		html += '</ul></div>';
